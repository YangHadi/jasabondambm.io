...
(function(html) {
    ...
    * ---------------------------------------------------- */
    const ssMobileMenu = function() {
        ...
        const mainNavWraps = document.querySelectorAll('.header-nav');
        const siteBody = document.querySelector('body');

        if (!(toggleButton && mainNavWraps.length)) return;
        ...
        toggleButton.addEventListener('click', function(e) {
            ...
            // toggle visibility for all navs
            mainNavWraps.forEach(nav => {
            ...
            nav.querySelectorAll('a').forEach(link => {
                link.addEventListener("click", function(event) {
                    if (window.matchMedia('(max-width: 900px)').matches) {
                        ...
                        mainNavWraps.forEach(n => n.classList.remove('is-open'));
                    }
                });
            });
        });
   ...
   /* highlight active services link on pagescroll
    * ------------------------------------------------------ */
